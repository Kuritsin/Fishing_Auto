...
def test_fast_downward_dive_is_a_bite():
    positions = [(100, 200)] * 24 + [(100, 203), (100, 210), (100, 216)]
...
def test_disappearance_after_started_drop_is_a_bite():
    ...
    feed(detector, [(100, 200)] * 24 + [(100, 204), (100, 211)])
...
def test_large_early_jump_is_not_a_bite_before_baseline_is_ready():
    positions = [(100, 200)] * 8 + [(100, 260), (100, 270), (100, 280)]
    assert not any(result.detected for result in feed(BiteSignalDetector(50), positions))